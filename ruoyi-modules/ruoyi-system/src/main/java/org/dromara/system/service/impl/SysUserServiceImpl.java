...
public class SysUserServiceImpl implements ISysUserService, UserService {
    ...
    public int insertUser(SysUserBo user) {
        ...
        // 如果用户类型为供应商 那么对应添加数据到供应商表中  具体的判断方式还需要修改
        if (user.getRoleIds()[0] == 1736674132956827649L) {
            SupplierBo bo = new SupplierBo();
            bo.setIphone(sysUser.getPhonenumber());
            bo.setUserid(String.valueOf(sysUser.getUserId()));
            bo.setName(sysUser.getUserName());
            bo.setStype(1L);
            supplierMapper.insert(MapstructUtils.convert(bo, Supplier.class));
        }
